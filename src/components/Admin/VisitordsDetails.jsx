...
const VisitorsDetails = () => {
  ...
  const handleExcelExport = () => {
    ...
    saveAs(new Blob([excelBuffer], { type: "application/octet-stream" }), "visitors.xlsx");
  };
 ...
 const filteredExportData = filteredVisitors.map(
  ...
  const columns = [
    ...
    { name: "Visitor Type", selector: (row) => row.visitorType, sortable: true },
    { name: "Officer", selector: (row) => row.officer, sortable: true },
  ...
  const handlePrint = () => {
  const theadTh = `
    <th>Name</th><th>Address</th><th>ID No</th><th>Phone</th>
    <th>Company</th><th>Vehicle</th><th>Type</th><th>Officer</th>
    <th>Dept</th><th>Purpose</th><th>Time In</th><th>Time Out</th>
    <th>Duration</th>
  `;
  ...
  const rowsHtml = filteredVisitors
    .map(
      (v) => `<tr>
        ...
        <td>${
          v.timeOut ? format(new Date(v.timeOut), "dd/MM/yyyy HH:mm") : "—"
        }</td>
        <td>${v.duration || "-"}</td>
      </tr>`
    )
    ...
    <html>
      <head>
        ...
        <style>
          ...
          .logo { width:80px;height:80px;border-radius:50%;object-fit:cover;display:block;margin:0 auto 10px; }
      ...
      <body>
        <img src="https://agroduka.ke/images/thumbnails/240/240/logos/8/6d0My9wH_400x400.jpg" class="logo" />
        ...
        <div class="footer">
          Generated by Ultravetis Visitor Pass System
        </div>
      </body>
    </html>
  `;
